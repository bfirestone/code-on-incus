...
def test_clean_force_skips_confirmation(coi_binary, cleanup_containers):
    ...
    assert result.returncode == 0, \
        f"coi clean --force should succeed. stderr: {result.stderr}"

    # Should not contain prompts for confirmation
    combined_output = result.stdout + result.stderr
    no_prompt = (
        "y/n" not in combined_output.lower() and
        "confirm" not in combined_output.lower() and
        "are you sure" not in combined_output.lower()
    )

    assert no_prompt, \
        f"--force should skip confirmation prompts. Got:\n{combined_output}"
